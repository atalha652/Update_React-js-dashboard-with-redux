...
class Card extends Component {
    ...
    renderActions(){
        ...
        if(this.props.collapse){
            actions.push(<a key="0" className={this.state.isOpen ? "list-icons-item" : "list-icons-item rotate-180"} data-action="collapse" onClick={this.toggleCollapse.bind(this)}></a>);
        }
        if(this.props.reload){
            actions.push(<a key="1" className="list-icons-item" data-action="reload" onClick={this.props.onReload}></a>);
        }
        if(this.props.remove){
            actions.push(<a key="2" className="list-icons-item" data-action="remove"></a>);
        }
        ...
        if(this.props.headerMultiselect){
            var select = this.props.headerMultiselect;
            actions.push(
                <div className="wmin-200" key="3" title={this.props.headerSelectPopover}>
                    <MultiSelect
                        className="c-form-control-sm"
    ...
    render() {
        ...
        if(this.props.showHeader && this.props.collapse && this.props.headerClickable===false){
            header = <div className={this.state.headerClass}>
                        <h5 className={this.state.headerTitleClass}>{this.props.header}</h5>
                        {moreactions}
                        {actions}
                    </div>
        }
